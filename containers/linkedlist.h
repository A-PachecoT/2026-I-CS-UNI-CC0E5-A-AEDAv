...
// Linked List Node (declarado primero para que los iteradores y traits puedan referenciarlo)
...
// Traits de Ordenamiento (refactorizadas a BaseTrait)
template <typename T>
...
protected:
    ...
    void internal_insert(Node* &pPrev, const value_type &value, Ref ref){
        if (!pPrev || m_comp(value, pPrev->getDataRef())){
            ...
            m_size++;
            if (pPrev->getNext() == nullptr) m_tail = pPrev;
            return;
        }
...
public:
    ...
    friend ostream& operator<<(ostream& os, LinkedList& list) {
        return os << static_cast<const LinkedList&>(list);
    }
